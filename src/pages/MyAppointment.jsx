      ...
      <div>
        {
         appointments.map((item,index) => (
            <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
                ...
                {!item.cancelled && !item.payment && !item.isCompleted && <button onClick={() => makePayment(item.docId)} className=' text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-primary hover:text-white transition-all duration-300'>Pay Online</button>}
                 ...
                 {item.cancelled && !item.isCompleted &&  <button className='text-sm text-red-500 text-center sm:min-w-48 py-2 border rounded'> Appointment Cancelled</button>}
